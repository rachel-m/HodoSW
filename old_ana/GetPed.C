// rmontgom@jlab.org - 12/04/21
// macro to fit pedestal part of adc spectra and write results to text file
// works using the replayed file from sbs offline
// check the location of the replayed file on line 49 before executing
// aug 2021, updated for new sbs-offline output file format and changed format of text file to match db file field for pedestal
#include <TROOT.h>
#include <TGraph.h>
...
void GetPed(const TString InFile="bbhodo_307_1000", Int_t nevents=-1,
  ...
  // RunNo is the run number
  // nevents is how many events to analyse, -1 for all
  // GaussSigma is initial guess in adc bin for sigma of gauss fit
  // FitRange is how many +-sigma the fit range convers
  // Cut is how many sigma above the pedestal mean you want the pedestal correction to lie
  // HistRange is the upper value to plot on x axis of adc spectrum
  // Display canvas =1 if you want to plot histos immediately, set to any other value for off
  ...
  // pedtextfile.open( TString::Format("AnalysedFiles/PedFits_replayed_bbhodo_%d.evio.0.txt", RunNo));
  ...
  // ADC histos
  TH1F *hADCL[nBars];
  for(Int_t adcchan=0; adcchan<(nBars); adcchan++){
    hADCL[adcchan] = new TH1F(TString::Format("hADC_Bar%d_L",adcchan),
			      TString::Format("hADC_Bar%d_L",adcchan),
			      NAdcBins, AdcBinLow, AdcBinHigh);
  }
  TH1F *hADCR[nBars];
  for(Int_t adcchan=0; adcchan<(nBars); adcchan++){
    hADCR[adcchan] = new TH1F(TString::Format("hADC_Bar%d_R",adcchan),
			      TString::Format("hADC_Bar%d_R",adcchan),
			      NAdcBins, AdcBinLow, AdcBinHigh);
  }
  ...
  for(Int_t bar=0; bar<(nBars); bar++){
    ...
    // define the fits
    fGaussL[bar] = new TF1(TString::Format("fGaus_Bar%d_L",bar),
  			      "gaus", fitlow, fithigh);
    // ped fit parameters
